...
impl ModelConfig {
    ...
    fn model_streaming_ttft_timeout(&self) -> Option<Duration> {
        self.timeouts.streaming.ttft_ms.map(Duration::from_millis)
    }

    fn model_streaming_total_timeout(&self) -> Option<Duration> {
        self.timeouts.streaming.total_ms.map(Duration::from_millis)
    }
    ...
    pub async fn infer<'request>(
        ...
        self.infer_with_request_timeouts(request, clients, model_name, None)
            .await
    }
    ...
    pub async fn infer_stream<'request>(
        ...
        self.infer_stream_with_request_timeouts(request, clients, model_name, None)
            .await
    }
...
mod tests {
    ...
    fn test_request_timeouts_override_configured_timeouts() {
        ...
        let request_timeouts = TimeoutsConfig {
            non_streaming: NonStreamingTimeouts { total_ms: None },
            streaming: StreamingTimeouts {
